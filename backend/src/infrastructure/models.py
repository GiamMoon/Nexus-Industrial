from sqlalchemy import Column, String, Boolean, DateTime, ForeignKey, Numeric, Integer, Text
from sqlalchemy.dialects.postgresql import UUID as PG_UUID
from sqlalchemy.orm import relationship
from pgvector.sqlalchemy import Vector  # <--- HABILITA IA VECTORIAL
import uuid
from datetime import datetime

from .database import Base

# --- MODELOS DE ACCESO ---

class UsuarioModel(Base):
    """Tabla para empleados (Nexus Control)"""
    __tablename__ = "usuarios_empleados"
    
    id = Column(PG_UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    email = Column(String, unique=True, index=True, nullable=False)
    password_hash = Column(String, nullable=False)
    nombre = Column(String)
    rol = Column(String, default="VENDEDOR") # RBAC simple
    activo = Column(Boolean, default=True)
    
    # Tracking GPS (REQ-ADMIN-04)
    ultima_latitud = Column(Numeric(10, 6), nullable=True)
    ultima_longitud = Column(Numeric(10, 6), nullable=True)
    ultima_actualizacion_gps = Column(DateTime, nullable=True)

class ClienteModel(Base):
    """Tabla para compradores (Nexus Market)"""
    __tablename__ = "clientes_web"
    
    id = Column(PG_UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    ruc_dni = Column(String(11), unique=True, index=True, nullable=False)
    razon_social = Column(String, nullable=False)
    email = Column(String, unique=True, index=True)
    password_hash = Column(String) # Login "Mi Cuenta"
    telefono_whatsapp = Column(String, nullable=True) # Para Webhook
    
    ventas = relationship("VentaModel", back_populates="cliente")

# --- MODELOS DE NEGOCIO ---

class ProductoModel(Base):
    __tablename__ = "productos"
    
    id = Column(PG_UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    sku = Column(String, unique=True, index=True)
    nombre = Column(String, index=True)
    descripcion = Column(Text) # Para humanos
    
    # --- CEREBRO IA ---
    # Vector de 1536 dimensiones (compatible con OpenAI ada-002)
    embedding_vector = Column(Vector(1536)) 
    
    precio_base = Column(Numeric(10, 2))
    precio_dinamico = Column(Numeric(10, 2)) # Calculado por IA
    stock = Column(Integer, default=0)
    
    # Url imagen para el E-commerce
    imagen_url = Column(String, nullable=True)

class VentaModel(Base):
    __tablename__ = "ventas"
    
    id = Column(PG_UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    fecha = Column(DateTime, default=datetime.utcnow)
    
    # Relaciones
    cliente_id = Column(PG_UUID(as_uuid=True), ForeignKey("clientes_web.id"), nullable=True)
    vendedor_id = Column(PG_UUID(as_uuid=True), ForeignKey("usuarios_empleados.id"), nullable=True)
    
    cliente = relationship("ClienteModel", back_populates="ventas")
    
    # Estado (REQ-EXT-01: Actualizable por WhatsApp)
    estado = Column(String, default="PENDIENTE_PAGO") # PENDIENTE, PAGADO, ENVIADO, ENTREGADO
    
    total = Column(Numeric(10, 2), default=0.0)
    
    # Datos SUNAT (REQ-TEC-02)
    xml_generado = Column(Text, nullable=True)
    cdr_sunat = Column(Text, nullable=True) # Constancia de Recepción
    hash_firma = Column(String, nullable=True)

class DetalleVentaModel(Base):
    __tablename__ = "detalle_ventas"
    
    id = Column(PG_UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    venta_id = Column(PG_UUID(as_uuid=True), ForeignKey("ventas.id"))
    producto_id = Column(PG_UUID(as_uuid=True), ForeignKey("productos.id"))
    
    cantidad = Column(Integer)
    precio_unitario = Column(Numeric(10, 2)) # Precio en el momento de la compra
    subtotal = Column(Numeric(10, 2))