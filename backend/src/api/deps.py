...
def get_current_employee(current_user: dict = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    print(f"🛡️ GUARDIA EMPLEADO: Verificando usuario {current_user['id']}...")
    ...
    # Lista de roles permitidos (NORMALIZAMOS A MAYÚSCULAS)
    roles_permitidos = ["ADMIN", "EMPLEADO", "SUPERVISOR", "ALMACENERO"]
    
    # Convertimos lo que viene de la BD a mayúsculas para comparar
